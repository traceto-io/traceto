...
All notable changes to Traceto are documented here.
...
- Mobile layout showed only "With Traceto" pane — `.pane-before` changed from `display: none` to `display: block; opacity: 0.45`
...
- `TracetoClient.shutdown(timeout)` method — flushes remaining queue and closes HTTP client cleanly; registered via `atexit` automatically
...
- typer-based CLI (`traceto init`, `traceto generate`, `traceto status`)
